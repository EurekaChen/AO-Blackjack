<script lang="ts">
	...
	function restore(aoPlayer:AOPlayer ) {
		...
		// for (let card of aoPlayer.state.hands[0].cards) {
		...
		// 	//有两手：
		// 	//恢复第二手
		// 	for (let card of aoPlayer.state.hands[1].cards) {
		...
		// 		$Action.newHand=true;
		// 	}
		// 	else if(aoPlayer.state.activeHandIndex==1){
		// 		//第一手活动：			
		// 	}
		// } else {
		// 	//一手时判断是否可拆牌
		...
		// 	if (aoPlayer.state.dealerCards[0].charAt(0) == 'A') {
		// 		//一手时可下保险
		// 		if (aoPlayer.state.insurance > 0) {
		// 			//已经下保险
		...
		// }
		//$Player=$Player;
		// const hand1CanDouble=aoPlayer.state.activeHandIndex==1 && aoPlayer.state.hands[0].cards.length==2;
		// const hand2CanDouble=aoPlayer.state.activeHandIndex==2 && aoPlayer.state.hands[1].cards.length==2;
